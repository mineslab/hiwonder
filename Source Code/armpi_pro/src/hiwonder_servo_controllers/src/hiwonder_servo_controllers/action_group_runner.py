...
class ActionGroupRunner:
    ...
    """
        check whether the file is a qualified action group file.
    """
    ...
    def process_action_group_run(self, goal):
        ...
        ''' All action files end with ".json". if not, the command with parameters to be imported need to use '|'to sperate the parameter fron command'''
         print(name)
        print(repeat)
        success = 0
        with self.lock:
